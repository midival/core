import {
  fractionToPitchBend,
  fractionToPitchBendAsUints,
  splitNumberIntoUInt8s,
  splitValueIntoFraction,
  uIntsIntoNumber,
} from "./pitchBend";
...
describe("Pitch bend utils", () => {
  ...
  it("should properly transform array to value from range -1 - 1.0", () => {
    ...
    expect(splitValueIntoFraction(fractionToPitchBendAsUints(0))).toBeCloseTo(
      0
    );
    expect(splitValueIntoFraction(fractionToPitchBendAsUints(1))).toBeCloseTo(
      1
    );
  });
});
